...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		String API_KEY = "1e6deaa8b4daca65e48005e73865e89d";
		...
		NodeList windSpeed = doc.getElementsByTagName("speed");
		...
		for (int date = 0; date < lastUpdatelist.getLength(); date++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				// and print it
				System.out.println(wBean.getCityStr() + " is now a " + XMLSunRise);
				// save it
				wBean.setDate(XMLSunRise);
//datum
			}
		}
		// loop through the content of the tag
				for (int wind = 0; wind < windSpeed.getLength(); wind++) {
					// Save a node of the current list id
					Node node = windSpeed.item(wind);
					if (node.getNodeType() == Node.ELEMENT_NODE) {

						// set the current node as an Element
						Element eElement = (Element) node;
						// get the content of an attribute in element
						String XMLWindSpeed = eElement.getAttribute("name");
						// and print it
						System.out.println(wBean.getCityStr() + " is now a " + XMLWindSpeed);
						// save it
						wBean.setWind(XMLWindSpeed);
		//wind
					}
				}
